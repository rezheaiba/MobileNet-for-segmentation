...
from ADE.mit_semseg.config import cfg
...
from ADE.mit_semseg.dataset import TrainDataset
from model import my_modle
from utils.ade_dataset import ADESegmentation
...
def criterion(outputs, target):
    ...
    # 忽略target中值为255的像素，255的像素是目标边缘或者padding填充, weight=weight.to(outputs.device)
    # losses = nn.functional.cross_entropy(outputs, target, ignore_index=150, weight=weight.to(outputs.device))
    losses = nn.functional.cross_entropy(outputs, target, ignore_index=0, weight=weight.to(outputs.device))
...
def main(args):
    ...
    # dataset
    train_dataset = ADESegmentation(transforms=get_transform(train=True),
                                  if_train=True)
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               batch_size=batch_size,
                                               num_workers=num_workers,
                                               shuffle=True)
    val_dataset = ADESegmentation(transforms=get_transform(train=False),
                                  if_train=False)
    ...
    resultRoot = './runs-ADE20K'
    if not os.path.exists(resultRoot):
        os.makedirs(resultRoot)
    logger_file = "./{}/results{}.txt".format(resultRoot, datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    # 创建该日志文件
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for image,target in train_loader:
            ...
            optimizer.zero_grad()
            # optimizer报错是因为没有进行梯度计算，就清零了。没有梯度计算因为loss没有计算？
            loss.backward()
            optimizer.step()
        ...
        save_file = {"model": model.state_dict(),
                     ...
                     "epoch": epoch,
                     "args": args}
        torch.save(save_file, "weights-ADE20K/model_{}.pth".format(epoch))
...
def parse_args():
    ...
    parser.add_argument("--epochs", default=10, required=False, type=int, metavar="N",
                        help="number of total epochs to train")
    parser.add_argument('--lr', default=0.001, required=False, type=float, help='initial learning rate')
    parser.add_argument('--resume', default='', required=False,
